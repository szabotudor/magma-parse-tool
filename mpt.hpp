...
namespace mgm {
    ...
    struct Result {
        ...
        template<std::enable_if_t<!std::is_same_v<T, E>, bool> = true>
        Result(T&& value) : _is_error{false} {
            new (&_result.value) T{ std::move(value) };
        }
        template<std::enable_if_t<!std::is_same_v<T, E>, bool> = true>
        Result(const T& value) : _is_error{false} {
            new (&_result.value) T{ value };
        }
        template<std::enable_if_t<!std::is_same_v<T, E>, bool> = true>
        Result(const E& error) : _is_error{true} {
    ...
    struct Pair {
        ...
        void emplace_a(Ts&&... ts) {
            a.~T();
            new (&a) T{std::forward(ts)...};
        }
        template<typename... Us, std::enable_if_t<std::is_constructible_v<U, Us...>, bool> = true>
        void emplace_b(Us&&... ts) {
            b.~U();
    ...
    class System {
        ...
        struct Source {
            struct SourceData {
                ...
                SourceData(const char* const& data_to_copy, const size_t size) : data{new char[size]}, size{size + 1}, owned{true} {
                ...
                SourceData sub_source(const size_t pos) const {
                    return SourceData{data + pos, size - pos};
                }

                std::string substr(const size_t pos, const size_t len) const {
                    return std::string{data + pos, len};
                }
            ...
            Source(const std::string& source, const SourcePos& pos = SourcePos{}) : source{source.c_str(), source.size()}, pos{pos} {}
        ...
        static Pair<size_t, size_t> get_first_word(const Source& str, bool full_brace) {
            ...
            switch (mode) {
                ...
                case NUM: {
                    while (is_num(str[res.b]) || str[res.b] == '.' || str[res.b] == 'u'
                    || str[res.b] == 'i' || str[res.b] == 'f')
                        ++res.b;
                    return res;
                    break;
                }
                ...
                case SYM: {
                    switch (str[res.a]) {
                        ...
                        case '"': {
                            ++res.b;
                            while ((str[res.b] != '"' || (str[res.b] == '"' && str[res.b - 1] == '\\')) && str[res.b] != '\0') ++res.b;
                            if (str[res.b] == '"') ++res.b;
                            return res;
                        }
                        default: {
        ...
        struct Rule {
            struct Word {
                ...
                Result<RepeatType> repeat() const {
                    if (empty())
                        return Error{-1, "Word is empty"};
                    switch (word[1]) {
                        case ' ': return RepeatType::ONCE;
                        case '*': return RepeatType::REPEAT;
                        case '#': return RepeatType::REPEAT_SINGLE;
                        default: return Error{-1, "Invalid repeat type"};
                    }
                }
                Result<Type> type() const {
                    if (empty())
                        return Error{-1, "Word is empty"};
                    switch (word[2]) {
                        case ' ': return Type::DIRECT;
                        case '$': return Type::GENERIC;
                        case '+': return Type::EXPAND;
                        case '!': return Type::ERROR_MESSAGE_SET;
                        case '?': return Type::ERROR_FIX_SET;
                        default: return Error{-1, "Invalid type"};
                    }
                }
                ...
                Word(const std::string& str) : word{str} {
                    if (optional().is_error() || repeat().is_error() || type().is_error())
                        word.clear();
                    if (type().result() == Type::EXPAND && (repeat().result() != RepeatType::ONCE || optional().result() != OptionalType::MANDATORY))
                        word.clear();
                }

                Word(const std::string name, const OptionalType optional, const RepeatType repeat, const Type type) : word{"   " + name} {
                    switch (optional) {
                    ...
                    switch (repeat) {
                        ...
                        case RepeatType::REPEAT: word[1] = '*'; break;
                        case RepeatType::REPEAT_SINGLE: word[1] = '#'; break;
                    }
                    switch (type) {
                        case Type::DIRECT: break;
                        case Type::GENERIC: word[2] = '$'; break;
                        case Type::EXPAND: word[2] = '+'; break;
                        case Type::ERROR_MESSAGE_SET: word[2] = '!'; break;
                        case Type::ERROR_FIX_SET: word[2] = '?'; break;
                    }
                }
            };
            ...
            Result<bool> is_valid() const {
                ...
                for (size_t i = 0; i < words.size(); i++) {
                    ...
                    if (word.repeat().result() != Word::RepeatType::ONCE && words[i + 1].optional().result() != Word::OptionalType::MANDATORY)
                        return Error{6, "Invalid rule. Any repeating word (or list of repeating words) cannot be followed by an optional word"};
            ...
            private:
            Result<Pair<size_t, size_t>> ensure_word_match(const Source& str, const size_t word_id, size_t* found_word_b_return = nullptr) const {
                ...
                switch (type) {
                    ...
                    case Word::Type::GENERIC: {
                        ...
                        if (i > 0) while (is_whitespace(str[i - 1])) --i;
                        first_word.b = i;
            ...
            Result<std::vector<WordMatch>, CompilationError> match(const Source& str) const {
                if (str.empty())
                    return CompilationError{{}, "String is empty", CompilationError::Severity::ERROR};
                const auto valid = is_valid();
                if (valid.is_error())
                ...
                while (i < words.size()) {
                    ...
                    if (word_match.is_error()) {
                        ...
                        if (words[i].repeat().result() == Word::RepeatType::REPEAT_SINGLE) {
                            if (!repeating && words[i].optional().result() != Word::OptionalType::OPTIONAL)
                                return CompilationError{(str + pos).pos, "Single repeating word not found", CompilationError::Severity::ERROR};
                            repeating = false;
                            ++i;
                            continue;
                        }
                        if (repeating) {
                            ...
                            return CompilationError{(str + pos).pos, "Repeating word not found or no closer was found after repeating words", CompilationError::Severity::ERROR};
                        }
                    ...
                    switch (words[i].repeat().result()) {
                        ...
                        case Word::RepeatType::REPEAT: ++i; repeating = true; break;
                        case Word::RepeatType::REPEAT_SINGLE: repeating = true; break;
                    }
                }
                return res;
            }
                ...
                for (size_t i = expr_to_expand.a + 1; i < expr_to_expand.b - 1; i++) {
                    if (str[i] == '$') {
                        ...
                        if (str[word_to_expand.a] == '(') {
                            const auto expand_result = expand_generic(str.substr(word_to_expand.a, word_to_expand.b - word_to_expand.a), expand_vars);
